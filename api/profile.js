...
router.put("/unfollow/:userToUnfollowId", authMiddleware, async (req, res) => {
  try {
    ...
    if (isFollowing) return res.status(401).send("User not followed before");
...
router.post("/update", authMiddleware, async (req, res) => {
  try {
    const { userId } = req;
    const { bio, twitter, instagram, profilePicUrl } = req.body;
    let profileFields = {};
...
router.post("/settings/messagePopup", authMiddleware, async (req, res) => {
  try {
    ...
    if (user.newMessagePopup) user.newMessagePopup = false;
    else user.newMessagePopup = true;
